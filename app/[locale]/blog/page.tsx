...
export default async function BlogPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] pt-20 pb-24 px-6">
          ...
          {posts.map((post) => {
            ...
            return (
              <Link key={post.id} href={`/blog/${post.slug}`} className="group block">
                <article className="h-full bg-[#12121a] border border-white/10 rounded-2xl overflow-hidden hover:border-indigo-500/40 transition-colors">
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <span className="absolute top-3 left-3 text-xs font-medium px-3 py-1 rounded-full bg-indigo-600/80 text-white">
                      {categoryLabel}
                    </span>
                  </div>
                  <div className="p-6">
                    ...
                    <h2 className="font-heading text-lg font-semibold text-white mb-3 line-clamp-2 group-hover:text-indigo-400 transition-colors">
                      {tr.title}
                    </h2>
                    <p className="text-stone-400 text-sm leading-relaxed line-clamp-3 mb-4">{tr.excerpt}</p>
                    <span className="text-indigo-400 text-sm font-medium">{t('readMore')} →</span>
                  </div>
                </article>
              </Link>
            );
